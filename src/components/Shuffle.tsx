    ...
    () => {
      ...
      const build = () => {
        ...
        chars.forEach(ch => {
          ...
          const w = (ch as HTMLElement).getBoundingClientRect().width;
          const h = (ch as HTMLElement).getBoundingClientRect().height;
          if (!w) return;
          ...
          wrappersRef.current.push(wrap as HTMLSpanElement);
        });
      };
      ...
      const play = () => {
        ...
        const tl = gsap.timeline({
          ...
          onRepeat: () => {
            ...
            if (isVertical) {
              gsap.set(strips, { y: (_i: number, t: Element) => parseFloat((t as HTMLElement).getAttribute('data-start-y') || '0') });
            } else {
              gsap.set(strips, { x: (_i: number, t: Element) => parseFloat((t as HTMLElement).getAttribute('data-start-x') || '0') });
            }
            onShuffleComplete?.();
          },
          onComplete: () => {
        ...
        const addTween = (targets: Element[], at: number | string) => {
          ...
          if (isVertical) {
            vars.y = (_i: number, t: Element) => parseFloat((t as HTMLElement).getAttribute('data-final-y') || '0');
          } else {
            vars.x = (_i: number, t: Element) => parseFloat((t as HTMLElement).getAttribute('data-final-x') || '0');
          }
        ...
        if (animationMode === 'evenodd') {
          const odd = strips.filter((_, i) => i % 2 === 1).filter((s): s is Element => s !== null);
          const even = strips.filter((_, i) => i % 2 === 0).filter((s): s is Element => s !== null);
