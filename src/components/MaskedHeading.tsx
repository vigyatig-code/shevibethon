...
const MaskedHeading = ({
  ...
  const settingsRef = useRef<Record<string, number | boolean>>({});
  ...
  const place = useCallback(() => {
    ...
    const fs = s.fillScale as number;
    const maxX = Math.max(0, ((fs - 1) / 2) * W);
    const maxY = Math.max(0, ((fs - 1) / 2) * H);

    media.style.transform = `translate3d(${clamp(off.x, -maxX, maxX).toFixed(2)}px, ${clamp(off.y, -maxY, maxY).toFixed(2)}px, 0) scale(${fs})`;
  ...
  const sync = useCallback(() => {
    ...
    root.style.fontSize = `${clamp(root.clientWidth * (s.textScale as number), 20, 200).toFixed(1)}px`;
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      const dx = Math.sin(clock * 0.21) * (s.drift as number);
      const dy = Math.cos(clock * 0.17) * (s.drift as number) * 0.6;
    ...
    const onMove = (e: PointerEvent) => {
      const s = settingsRef.current;
      if ((s.parallax as number) <= 0) return;
      const r = root.getBoundingClientRect();
      const nx = ((e.clientX - r.left) / (r.width || 1)) * 2 - 1;
      const ny = ((e.clientY - r.top) / (r.height || 1)) * 2 - 1;
      offsetRef.current.tx = clamp(nx, -1, 1) * -(s.parallax as number);
      offsetRef.current.ty = clamp(ny, -1, 1) * -(s.parallax as number);
    };
