...
export default function RippleDistortion({
  ...
  const uniformsRef = useRef<Record<string, Record<string, { value: unknown }>> | null>(null)
  ...
  useEffect(() => {
    ...
    const compositeUniforms: Record<string, { value: number | number[] | [number, number, number] | Texture }> = {
    ...
    uniformsRef.current = { wave: waveUniforms as unknown as Record<string, { value: unknown }>, composite: compositeUniforms as unknown as Record<string, { value: unknown }> }
    ...
    const setNewWave = (x: number, y: number, power: number) => {
      const cfg = configRef.current as Record<string, number>
    ...
    const onMove = (event: PointerEvent) => {
      ...
      if (!point) return
      const step = Math.max(1, cfg.spacing as number)
    ...
    const onDown = (event: PointerEvent) => {
      ...
      const point = localPoint(event.clientX, event.clientY)
      if (!point) return
      setNewWave(point[0], point[1], Math.max(1, cfg.clickStrength as number))
    }
  ...
  useEffect(() => {
    ...
    if (!u) return
    const w = u.wave as { uRings: { value: number } }
    const c = u.composite as Record<string, { value: number | number[] }>
    w.uRings.value = rings
    c.uStrength.value = strength
    c.uSwirl.value = swirl
    c.uDispersion.value = dispersion
    c.uGlint.value = glint
    c.uTintAmount.value = tintAmount
    c.uGrayscale.value = grayscale ? 1 : 0
    c.uHighlight.value = hexToRGB(highlightColor)
    c.uTint.value = hexToRGB(tint)
