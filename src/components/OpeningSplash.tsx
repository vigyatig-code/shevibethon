...
export default function OpeningSplash({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    ...
    const totalTime = CIVIC_ISSUE_IMAGES.length * FLASH_DURATION + 200
    addTimer(startExit, totalTime)

    addTimer(startExit, totalTime + 3000)

    return cleanup
  }, [cleanup, startExit, onComplete])

  return (
    <div className={`civic-opening-splash ${isExiting ? 'exiting' : ''}`} aria-hidden="true">
